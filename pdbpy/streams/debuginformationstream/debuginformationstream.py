...
from pdbpy.streams.typestream.records.base import read_stringz
...
class PdbDebugInformationStream:
    ...
    def __init__(self, file: MultiStreamFileStream, stream_directory : StreamDirectoryStream, upfront_memory : bool = False, debug : bool=False):
        ...
        dbiex_area = bytes(self.file[c_sizeof(self.header) : c_sizeof(self.header) + self.header.module_size])

        ModuleInformation.from_memory(memoryview(dbiex_area), False)
